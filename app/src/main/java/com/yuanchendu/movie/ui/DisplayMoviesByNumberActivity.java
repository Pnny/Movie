...
public class DisplayMoviesByNumberActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_display_movies_by_number);
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (condition.getText().toString().equals("")){
                    ...
                    movieBeans.clear();
                    rateString = condition.getText().toString();
                    for (int i =0;i  <app.getMovieBeens().size();i++)
                    {
                        MovieBean bean = app.getMovieBeens().get(i);
                        float temp = Float.valueOf(bean.getRate());
                        if (temp >= Float.valueOf(rateString)){
                            movieBeans.add(bean);
                        }
                    }
                    adapter.notifyDataSetChanged();
